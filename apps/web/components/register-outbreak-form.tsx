...
export function RegisterOutbreakForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try { setCreated(await apiFetch<Outbreak>("/outbreaks", { method: "POST", body: JSON.stringify(payload) })); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not create outbreak record."); } finally { setSubmitting(false); }
  }
  ...
  if (error && locations.length === 0) return <ErrorMessage message={`Unable to load locations: ${error}`} />;
