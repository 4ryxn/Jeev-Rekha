...
const copy = {
  en: {
    ...
    check: "Check movement advisory", workspace: "Operations workspace", print: "Print advisory slip",
  ...
  hi: {
    ...
    check: "आवागमन सलाह जाँचें", workspace: "संचालन कार्यक्षेत्र", print: "सलाह पर्ची प्रिंट करें",
    ...
    <main className="min-h-screen bg-paper px-4 py-6 sm:px-8">
      <div className="mx-auto max-w-4xl">
        <header className="flex flex-col gap-4 border-b border-line pb-6 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.14em] text-teal">Synthetic demonstration · Pre-travel advisory</p>
          ...
          <div className="flex gap-2" aria-label="Language">
            <button onClick={() => setLanguage("en")} className={`min-h-11 rounded-lg border px-3 font-bold ${language === "en" ? "border-teal bg-teal text-white" : "border-line bg-white"}`}>English</button>
            <button onClick={() => setLanguage("hi")} className={`min-h-11 rounded-lg border px-3 font-bold ${language === "hi" ? "border-teal bg-teal text-white" : "border-line bg-white"}`}>हिन्दी</button>
          </div>
        </header>
        ...
        <form onSubmit={(event) => { event.preventDefault(); submit(event.currentTarget); }} className="mt-6 rounded-xl border border-line bg-white p-5 shadow-sm sm:p-7">
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <Field label={t.vaccination}><select name="vaccination" defaultValue=""><option value="" disabled>{t.selectVaccinationEvidence}</option><option value="verified">Verified</option><option value="declared">Declared</option><option value="unknown">Unknown</option></select></Field>
          </div>
...
function Result({ result, details, t }: { result: PublicMovementCheck; details: Details; t: (typeof copy)[Language] }) {
  ...
  return <section className="mt-6 rounded-xl border-2 border-ink bg-white p-6 sm:p-8"><p className="text-xs font-bold uppercase tracking-[.14em] text-teal">Jeev Rekha — Pre-Travel Advisory Slip · Synthetic demonstration</p><div className="mt-4 flex gap-4"><Icon className="shrink-0 text-teal" size={36} aria-hidden="true" /><div><p className="text-sm font-bold uppercase tracking-wider">{result.risk_state.toUpperCase()}</p><h2 className="mt-1 font-display text-3xl font-semibold">{t.statuses[result.risk_state]}</h2></div></div><p className="mt-5 text-sm font-semibold">{details.origin} → {details.destination} · {details.species} · {details.count} animals</p><div className="mt-6 grid gap-5 md:grid-cols-2"><div><h3 className="font-bold">{t.meaning}</h3><p className="mt-2 text-sm leading-6">{t.statuses[result.risk_state]}</p><p className="mt-3 text-sm">{t.coverage}: {result.evidence_coverage_score}/100 · {result.information_coverage}</p></div><div><h3 className="font-bold">{t.action}</h3><p className="mt-2 text-sm leading-6">{t.actions[result.risk_state]}</p></div></div><p className="mt-5 text-xs text-slate-600">{new Date(result.generated_at).toLocaleString()}</p><p className="mt-4 rounded-lg bg-paper p-4 text-sm font-semibold">{t.disclaimer}</p><button onClick={() => window.print()} className="mt-5 min-h-11 rounded-lg border border-ink px-4 font-bold">{t.print}</button></section>;
}
