...
export function WorkspaceSettings() {
  ...
  useEffect(() => {
    ...
    apiFetch<{ configured: boolean }>("/routing/status").then((value) => setRouting(value.configured)).catch(() => setRouting(false));
  }, []);
  ...
  return <div className="space-y-6">
    ...
    <Card className="p-5 md:p-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-teal">Routing configuration</p><h2 className="mt-2 font-display text-2xl font-semibold">Pilot road-route provider</h2><p className="mt-3 text-sm leading-6 text-slate-700">{routing === null ? "Checking routing configuration…" : routing ? "Road route provider configured" : "Road route provider unavailable"}. Provider calls remain server-side and road routes are informational only.</p></Card>
    <Card className="p-5 md:p-6"><p className="text-xs font-bold uppercase tracking-[.14em] text-teal">Pilot advisory policy</p><h2 className="mt-2 font-display text-2xl font-semibold">pilot-radius-rules-v1</h2><p className="mt-3 text-sm leading-6 text-slate-700">Red: confirmed configured review-area contact. Amber: suspected contact or unverified vaccination. Grey: missing route geometry, radius, or required evidence. Green: complete evidence with no active configured review-area contact.</p><p className="mt-3 text-sm font-semibold">Rules require authorised veterinary validation before operational use.</p></Card>
