...
import { type Advisory, type Consignment, type Outbreak, type OutbreakTrendPoint, apiFetch } from "@/lib/api";
...
import { OutbreakTrendChart } from "@/components/outbreak-trend-chart";
...
export function DashboardContent() {
  ...
  const [trends, setTrends] = useState<OutbreakTrendPoint[]>([]);
  const [error, setError] = useState("");

  useEffect(() => {
    Promise.all([apiFetch<Outbreak[]>(sourcePath("/outbreaks", context)), apiFetch<Consignment[]>(sourcePath("/consignments", context)), apiFetch<Advisory[]>(sourcePath("/advisories", context)), context === "all" ? Promise.resolve([] as OutbreakTrendPoint[]) : apiFetch<OutbreakTrendPoint[]>(`/outbreaks/trends?source=${context}`)])
      .then(([loadedOutbreaks, loadedConsignments, loadedAdvisories, loadedTrends]) => { setOutbreaks(loadedOutbreaks); setConsignments(loadedConsignments); setAdvisories(loadedAdvisories); setTrends(loadedTrends); setState("ready"); })
  ...
  return <div className="space-y-8"><DataContextLabel />
...
function OutbreakStatusLabel({ status }: { status: Outbreak["status"] }) { const labels = { confirmed: ["red", "Confirmed"], suspected: ["amber", "Suspected"], under_investigation: ["amber", "Under investigation"], closed: ["grey", "Closed"] } as const; const [state, label] = labels[status]; return <StatusBadge state={state} label={label} />; }
